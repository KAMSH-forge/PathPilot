...
class VoiceCommandHandler {
  ...
  final Function captureAndSendImage; // New callback for capturing images
  final Function getAiResponse; // New callback for getting AI responses
  ...
  VoiceCommandHandler({
    ...
    required this.captureAndSendImage,
  ...
  void processVoiceCommand(String recognizedText) {
    ...
    if (!_isValidCommand(text)) {
      // showFeedback("Please start your command with 'Pilot'.");
      return;
    }
    ...
    if (_handleTakeMeTo(command)) {
      ...
      return;
    } else if (_handleUserGuide(command)) {
      return;
    } else if (_handleCaptureImage(command)) {
      return;
    } else if (_handleGetAiResponse(command)) {
      return;
    } else {
      // processVoiceCommandWithAI(command);
      showFeedback("Try 'Pilot user guide'.");
    }
  }
  ...
  bool _handleGetAiResponse(String command) {
    if (command == "show ai response" || command == "ai feedback") {
      String aiResponse = getAiResponse(); // Call the passed-in method
      showFeedback("AI Response: $aiResponse");
      return true;
    }
    return false;
  }

  bool _handleUserGuide(String command) {
    // listen for “user guide” or “guide”
    final triggers = ["user guide", "guide", "help"];
    if (triggers.contains(command)) {
      userGuide();
      return true;
    }
    return false;
  ...
  void userGuide() {
    showFeedback("Path Pilot User Guide:\n"
        "• 'Pilot, take me to [destination]'\n"
        "• 'Pilot, where am I?'\n"
        "• 'Pilot, stop navigation'\n"
        "• 'Pilot, show camera'\n"
        "• 'Pilot, hide camera'\n"
        "• 'Pilot, switch view'\n"
        "• 'Pilot, user guide'\n");
  ...
  //     } else {
  //       print(
  //           "Error calling Google AI Studio: ${response.statusCode} ${response.body}");
  ...
  //                     .containsKey("text")) {
  //               final String aiResponseChunk = responseData["candidates"][0]
  ...
  //     } else {
  //       print(
  //           "Error calling Google AI Studio (Stream): ${streamedResponse.statusCode} ${await streamedResponse.stream.bytesToString()}");
  //       showFeedback("An error occurred while processing your command.");
  //     }
  //   } catch (e) {
